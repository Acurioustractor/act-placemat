-- Identity Map and LinkedIn Contacts Enhancements
-- Creates a canonical person identity map and links LinkedIn contacts
...
CREATE TABLE IF NOT EXISTS person_identity_map (
  ...
  full_name TEXT,
  email TEXT UNIQUE,
  linkedin_contact_id UUID, -- references linkedin_contacts(id) when available
  gmail_id TEXT,
  notion_id TEXT,
...
-- Optional FK (leave NOT VALID to avoid failing on existing rows; can be validated later)
DO $$ BEGIN
  ALTER TABLE linkedin_contacts
...
-- Convenience view: high-value contacts with identity
