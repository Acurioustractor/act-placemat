/**
 * GoalsCalendarView - Calendar view for goals with due dates
 *
 * Features:
 * - Month navigation
...
// LCAA Lane colors (matching GoalsDashboard)
...
export function GoalsCalendarView({
  goals,
  onGoalClick,
  onLaneFilterChange,
  currentLaneFilter,
}: GoalsCalendarViewProps) {
  ...
  const getGoalsForDate = (date: Date): Goal[] => {
    ...
    return goals.filter(goal => {
      ...
      // Match date portion only (ignore time)
  ...
  const goalsWithDueDates = useMemo(() =>
    goals.filter(g => g.due_date), [goals]
  )
      ...
      {onLaneFilterChange && (
        <div style={filterRowStyle}>
          ...
          <div style={filterButtonsStyle}>
            ...
            {Object.keys(LANE_COLORS).filter(l => !l.includes(' — ')).map(lane => (
              <button
                key={lane}
                onClick={() => onLaneFilterChange(lane)}
                style={{
                  ...filterButtonStyle,
                  borderLeftColor: LANE_COLORS[lane],
                  ...(currentLaneFilter === lane ? activeFilterStyle : {}),
                }}
              >
                {lane}
              </button>
      ...
      <div style={sidePanelStyle}>
        ...
        {overdueGoals.length > 0 && (
          <div style={sectionStyle}>
            <h4 style={sectionTitleStyle}>⚠️ Overdue</h4>
