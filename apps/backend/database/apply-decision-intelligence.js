...
/**
 * Apply Decision Intelligence Schema to Supabase via exec_sql
...
async function main() {
  console.log('🚀 Applying Decision Intelligence schema...');
  ...
  // Apply the entire file in one call to preserve function bodies and ordering
  const { error } = await supabase.rpc('exec_sql', { query: sql });
  if (error) {
    console.error(`❌ Failed to apply Decision Intelligence schema: ${error.message}`);
    process.exit(1);
  }
