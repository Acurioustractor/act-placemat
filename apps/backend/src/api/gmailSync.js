...
router.get('/status', asyncHandler(async (req, res) => {
  try {
    ...
    // Check if service is properly initialized and has auth status
    const status = service.getStatus ? service.getStatus() : { authenticated: false, hasTokens: false };
    ...
    try {
      ...
      console.log('Could not get Gmail stats:', statsError.message);
      stats = {
        totalProcessed: 0,
        communityEmails: 0,
        contactsSync: 0,
    ...
    res.json({
      success: true,
      connected: status.authenticated || false,
      authenticated: status.authenticated || false,
      hasTokens: status.hasTokens || false,
      monitoring: service.isMonitoring || false,
      email: service.connectedEmail || null,
      lastSync: stats.lastSync || null,
      stats: {
        totalEmailsProcessed: stats.totalProcessed || 0,
        communityEmailsDetected: stats.communityEmails || 0,
        contactsSync: stats.contactsSync || 0,
        lastSync: stats.lastSync || null
      },
      setupRequired: !status.authenticated,
      setupInstructions: !status.authenticated ? [
        'Run "cd apps/backend && node setup-gmail-once.js"',
        'Follow OAuth authentication flow',
        'Return to dashboard and refresh'
      ] : null
    });
  } catch (error) {
    // Return safe status even on error
    res.json({
      success: false,
      connected: false,
      authenticated: false,
  ...
  try {
    ...
    res.json({
      ...
      emails: emails.map(email => ({
        ...
        communityContact: email.community_contact_id ? {
          id: email.community_contact_id,
          name: email.contact_name
        } : null,
        mentionedProjects: email.mentioned_projects || []
      }))
    });
  } catch (error) {
    res.status(500).json({
