...
export class SmartGmailSyncService extends EventEmitter {
  ...
  async loadStoredTokens() {
    try {
      // Try loading from file first (same as ProductionGmailService)
      const fs = await import('fs/promises');
      const path = await import('path');
      const tokenFile = path.join(process.cwd(), '.gmail_tokens.json');
      
      const tokensJson = await fs.readFile(tokenFile, 'utf8');
      ...
      try {
        ...
        // Ignore and try in-memory fallback
      }
      
      // Final fallback to global memory
  ...
  async getCommunityEmails(options = {}) {
    try {
      ...
      // This would query the database for community emails
      console.log(`📧 Getting ${limit} community emails from last ${days} days`);
