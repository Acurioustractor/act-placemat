...
class NotionService {
  ...
  async initializeClient() {
    try {
      ...
      if (oauthToken) {
        ...
        console.log('✅ Notion OAuth client initialized');
        ...
        console.log('✅ Notion regular token client initialized');
      } else {
  ...
  async handleWebhookEvent(event) {
    try {
      ...
      if (data.database_id) {
        const dbType = this.getDatabaseTypeById(data.database_id);
        if (dbType) {
  ...
  /**
   * Enhanced Notion query method with v2025 features
   */
  async queryNotion(databaseId, filter = {}, sorts = [], pageSize = 100, options = {}) {
    ...
    return await this.withRetry(async () => {
      ...
      while (
        ...
        const result = await this.notion.databases.query(queryParams);
  ...
  async getProjects(useCache = true, filter = {}) {
    ...
    try {
      ...
      const results = await this.queryNotion(this.databases.projects.id, filter, sorts);
    ...
    try {
      ...
      if (results?.results) {
        results.results.forEach(page => {
          // Categorize results based on database ID or properties
          if (page.parent?.database_id === this.databases.partners) {
            searchResults.partners.push({
              id: page.id,
              name: this.extractTitle(page.properties.Name?.title || []),
              type: 'partner',
            });
          } else if (page.parent?.database_id === this.databases.projects) {
    ...
    return [
      {
        ...
        description:
          'Indigenous-led organization pioneering community-controlled development across remote Australia.',
        featured: true,
      },
    ];
  }
    ...
    return [
      {
        ...
        budget: 250000,
        tags: [{ name: 'Digital Platform' }, { name: 'Community Voice' }],
        featured: true,
      },
      {
        id: 'fallback-2',
        ...
        budget: 180000,
        tags: [{ name: 'Justice' }, { name: 'Network' }, { name: 'Reform' }],
        featured: true,
      },
      {
        id: 'fallback-3',
        ...
        budget: 320000,
        tags: [{ name: 'Indigenous' }, { name: 'Youth' }, { name: 'Advocacy' }],
        featured: true,
      },
      {
        id: 'fallback-4',
        ...
        budget: 150000,
        tags: [{ name: 'Economic' }, { name: 'Regional' }, { name: 'Empowerment' }],
        featured: false,
      },
      {
        id: 'fallback-5',
        ...
        budget: 75000,
        tags: [{ name: 'Art' }, { name: 'Community' }, { name: 'Healing' }],
        featured: false,
      },
      {
        id: 'fallback-6',
        ...
        budget: 200000,
        tags: [{ name: 'Global' }, { name: 'Connections' }, { name: 'International' }],
        featured: true,
      },
    ];
  }
  ...
  async createProject(projectData) {
    ...
    try {
      ...
      const response = await this.notion.pages.create({
        parent: { database_id: this.databases.projects.id },
  ...
  async createOrganization(orgData) {
    ...
    try {
      ...
      const response = await this.notion.pages.create({
        parent: { database_id: this.databases.organizations.id },
        properties: orgProps
      });
