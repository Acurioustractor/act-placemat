...
import {
  ...
  ModeContext,
  ModeDetectionResult,
  ModeDetectionOptions,
} from './types.js';

/**
 * Mode definitions with characteristics
 */
export const MODES: Record<WorkModeKey, WorkMode> = {
  DEEP_WORK: {
    name: 'Deep Work',
    icon: '🎯',
...
export class ModeDetectorService {
  private dbConfigured: boolean = false;
  ...
  /**
   * Detect the optimal work mode based on current context
   *
   * @param options - Optional overrides for context
   * @returns Detection result with recommended mode and context
   */
  async detectMode(options: ModeDetectionOptions = {}): Promise<ModeDetectionResult> {
    ...
    // Score each mode
    const scores: Record<WorkModeKey, number> = {} as Record<WorkModeKey, number>;

    for (const [modeKey, mode] of Object.entries(MODES)) {
      scores[modeKey as WorkModeKey] = this.scoreMode(mode, context);
    }
    ...
    return {
      mode: recommendedMode as WorkModeKey,
      modeInfo: MODES[recommendedMode as WorkModeKey],
      score,
      confidence: score > 0.7 ? 'high' : score > 0.5 ? 'medium' : 'low',
      alternative: {
        mode: alternativeMode as WorkModeKey,
        modeInfo: MODES[alternativeMode as WorkModeKey],
        score: altScore,
      },
      context: {
        hour: context.hour,
      ...
      reasoning: this.generateReasoning(recommendedMode as WorkModeKey, context),
    };
  }
   ...
   */
  private scoreMode(mode: WorkMode, context: any): number {
    let score = 0;
  ...
  /**
   * Get calendar density (meeting count) for a specific date
   */
  private async getCalendarDensity(date: Date): Promise<number> {
  ...
  /**
   * Get recently used modes (for variety in recommendations)
   */
  private async getRecentModes(): Promise<WorkModeKey[]> {
  ...
  /**
   * Generate human-readable reasoning for the recommended mode
   */
  private generateReasoning(mode: WorkModeKey, context: any): string {
  ...
  /**
   * Get mode suggestions for different time slots throughout the day
   *
   * @returns Array of suggested modes for upcoming time slots
   */
  async getDaySuggestions(): Promise<Array<{
    time: string;
    mode: WorkModeKey;
    modeInfo: WorkMode;
    score: number;
    ...
    for (const hour of timeSlots) {
      ...
      if (slotTime > now) {
        ...
        let bestMode: WorkModeKey | null = null;
        let bestScore = 0;
        ...
        for (const [modeKey, mode] of Object.entries(MODES)) {
          ...
          if (score > bestScore) {
            bestScore = score;
            bestMode = modeKey as WorkModeKey;
          }
        }
