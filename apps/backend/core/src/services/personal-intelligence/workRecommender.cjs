 ...
 *
 * Usage:
 *   const { workRecommender } = require('./workRecommender');
 *   const recommendations = await workRecommender.getRecommendations();
 *   const patterns = await workRecommender.analyzePatterns();
 *
...
// Day patterns - learned from typical knowledge worker patterns
...
// Hour patterns - optimal task types by time of day
const DEFAULT_HOUR_PATTERNS = {
...
class WorkRecommenderService {
  constructor() {
    this.dbConfigured = { main: false, ghl: false };
    this.patterns = null;
  ...
  async initialize() {
    ...
    // Load patterns if we have history
  ...
  async loadPatterns() {
    ...
    try {
      // Try to load from sync_state table
  ...
  getPredictiveContext() {
    ...
    // Get hour pattern
    let hourPattern = DEFAULT_HOUR_PATTERNS.morning;
   ...
   */
  getTimeBasedRecommendation(dayPattern, hourPattern, hour) {
    const dayType = dayPattern.type;
    const hourType = hourPattern.type;
    ...
    return {
      message: `${dayType} day - focus on ${dayPattern.focus.join(', ')}`,
  ...
  async analyzePatterns() {
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        // Track what types of work happened on which days
      ...
      const learnedDayPatterns = {};
      for (const [day, logs] of Object.entries(dayStats)) {
        if (logs.length === 0) continue;

        // Find most common type for this day
        const typeCounts = {};
        const energyCounts = { high: 0, medium: 0, low: 0 };

        for (const log of logs) {
          typeCounts[log.type] = (typeCounts[log.type] || 0) + 1;
          energyCounts[log.energy.toLowerCase()] = (energyCounts[log.energy.toLowerCase()] || 0) + 1;
        }
      ...
      if (Object.keys(learnedDayPatterns).length > 0) {
        this.patterns = {
          ...
          hour: DEFAULT_HOUR_PATTERNS, // Keep default hour patterns
      ...
      return { error: err.message, patterns: this.patterns };
    }
  }
   ...
   */
  async recordCompletion(taskType, completedAt = new Date()) {
    const completion = {
      taskType,
  ...
  async getCalendarContext() {
    if (!this.dbConfigured.main) {
      return { events: [], freeBlocks: [], meetingCount: 0 };
    }
    ...
    try {
      ...
      return { events: [], meetings: [], meetingCount: 0, freeBlocks: [] };
    }
  }
   ...
   */
  calculateFreeBlocks(events, date) {
    const blocks = [];
    const workStart = 9;
    const workEnd = 18;
  ...
  async getActiveIssues() {
    ...
    try {
      ...
      const data = await response.json();
      const issues = (data.results || []).map((page) => this.extractIssueData(page));
  ...
  extractIssueData(page) {
    ...
    return {
      ...
      labels: props.Labels?.multi_select?.map((l) => l.name) || [],
      sprint: props.Sprint?.select?.name || null,
      assignee: props.Assignee?.people?.[0]?.name || null,
      isBlocking: (props.Labels?.multi_select || []).some((l) =>
  ...
  async getContactsNeedingAttention(daysThreshold = 25) {
    ...
    try {
      // Get last communication per contact
      ...
      const stale = Object.entries(contactLastComm)
        .filter(([_, lastDate]) => lastDate < threshold)
        .map(([contactId, lastDate]) => ({
          ghl_contact_id: contactId,
          last_communication: lastDate,
          days_since_contact: Math.floor((new Date() - lastDate) / (1000 * 60 * 60 * 24)),
        }))
   ...
   */
  scoreTasks(issues, mode, calendar, predictive = null) {
    const modeTaskAlignment = {
    ...
    // Get predictive recommendations if available
    ...
    return issues
      .map((issue) => {
        ...
        // Predictive alignment - boost tasks that match time patterns
        if (predictive) {
   ...
   */
  generateSuggestion(topTask, mode, calendar, predictive = null) {
    ...
    // Add predictive wisdom if available
  ...
  formatRecommendations(recommendations) {
    ...
    // Predictive Context (new section)
...
// Singleton instance
const workRecommender = new WorkRecommenderService();

module.exports = { workRecommender, WorkRecommenderService, DEFAULT_DAY_PATTERNS, DEFAULT_HOUR_PATTERNS };
module.exports.default = workRecommender;
