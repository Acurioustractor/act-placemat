...
export class ResearchIntelligenceOrchestrator {
  constructor() {
    ...
    // Free Research AI configuration (replaces Perplexity)
    this.researchConfig = {
      service: freeResearchAI,
      providers: ['tavily', 'serpapi', 'duckduckgo', 'groq'],
      fallback: true
    };
  ...
  async analyzeQueryPatterns(query, context) {
    ...
    const patternAnalysis = await this.anthropic.messages.create({
      model: 'claude-sonnet-4-5-20250929', // Upgraded to latest Sonnet 4.5
      max_tokens: 1000,
      temperature: 0.2,
      messages: [
        {
  ...
  async orchestrateMultiSourceResearch(query, patternAnalysis, depth) {
    ...
    // 1. Free Research AI (Real-time intelligence via Groq + Search APIs)
    researchTasks.push(this.conductFreeResearch(query, patternAnalysis));
    ...
    return {
      free_research: this.extractSettledValue(researchResults[0]),
