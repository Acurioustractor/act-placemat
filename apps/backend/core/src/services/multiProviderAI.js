  ...
  async transcribeAudio(audioBuffer, options = {}) {
    ...
    try {
      if (!this.providers.openai) {
        throw new Error('OpenAI provider not available for audio transcription');
      }
      ...
      const response = await this.providers.openai.audio.transcriptions.create({
        file: audioFile,
        model: 'whisper-1',
        language: options.language || 'en',
        prompt: options.prompt || 'This is a business note from an Australian user.',
        temperature: 0.1, // More accurate transcription
        response_format: 'text'
      });
      
      console.log(`✅ Audio transcribed: ${response.length} characters`);
      ...
      if (this.providers.groq && error.message.includes('rate limit')) {
        try {
          const groqAudioFile = new File([audioBuffer], 'audio.mp3', { 
            type: options.mimeType || 'audio/mpeg' 
          });
          
          const groqResponse = await this.providers.groq.audio.transcriptions.create({
            file: groqAudioFile,
          ...
          console.log('✅ Audio transcribed via Groq fallback');
          ...
          console.error('❌ Groq transcription fallback failed:', groqError);
        }
      }
