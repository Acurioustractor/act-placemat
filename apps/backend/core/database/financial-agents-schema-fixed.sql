-- Financial Agents Database Schema (Fixed for PostgreSQL)
...
CREATE INDEX IF NOT EXISTS idx_agent_actions_transaction_id ON agent_actions(transaction_id);
...
-- Create indexes for financial_exceptions
CREATE INDEX IF NOT EXISTS idx_financial_exceptions_status ON financial_exceptions(status);
CREATE INDEX IF NOT EXISTS idx_financial_exceptions_agent ON financial_exceptions(agent_name);
CREATE INDEX IF NOT EXISTS idx_financial_exceptions_type ON financial_exceptions(exception_type);
CREATE INDEX IF NOT EXISTS idx_financial_exceptions_priority ON financial_exceptions(priority);
...
-- Create indexes for manual_review_tasks
CREATE INDEX IF NOT EXISTS idx_manual_review_tasks_status ON manual_review_tasks(status);
CREATE INDEX IF NOT EXISTS idx_manual_review_tasks_agent ON manual_review_tasks(agent_name);
CREATE INDEX IF NOT EXISTS idx_manual_review_tasks_due_date ON manual_review_tasks(due_date);
...
CREATE INDEX IF NOT EXISTS idx_approval_requests_agent ON approval_requests(agent_name);
...
-- Create indexes for bank_transfers
CREATE INDEX IF NOT EXISTS idx_bank_transfers_transaction_id ON bank_transfers(transaction_id);
CREATE INDEX IF NOT EXISTS idx_bank_transfers_type ON bank_transfers(transfer_type);
CREATE INDEX IF NOT EXISTS idx_bank_transfers_source_account ON bank_transfers(source_account);
CREATE INDEX IF NOT EXISTS idx_bank_transfers_created_at ON bank_transfers(created_at);

-- Create xero_transactions table if it doesn't exist (extended from your existing financial API)
CREATE TABLE IF NOT EXISTS xero_transactions (
    id SERIAL PRIMARY KEY,
    xero_id VARCHAR(100) UNIQUE NOT NULL,
    date DATE NOT NULL,
    description TEXT,
    amount DECIMAL(12,2) NOT NULL,
    contact VARCHAR(255),
...
-- Create indexes for categorisation_rules
CREATE INDEX IF NOT EXISTS idx_categorisation_rules_pattern ON categorisation_rules(pattern);
CREATE INDEX IF NOT EXISTS idx_categorisation_rules_category ON categorisation_rules(category);
CREATE INDEX IF NOT EXISTS idx_categorisation_rules_active ON categorisation_rules(is_active);
...
-- Create indexes for rdti_activities
CREATE INDEX IF NOT EXISTS idx_rdti_activities_type ON rdti_activities(activity_type);
CREATE INDEX IF NOT EXISTS idx_rdti_activities_status ON rdti_activities(status);
CREATE INDEX IF NOT EXISTS idx_rdti_activities_project ON rdti_activities(project_code);
...
-- Create indexes for rdti_evidence
CREATE INDEX IF NOT EXISTS idx_rdti_evidence_activity ON rdti_evidence(activity_id);
CREATE INDEX IF NOT EXISTS idx_rdti_evidence_type ON rdti_evidence(evidence_type);
CREATE INDEX IF NOT EXISTS idx_rdti_evidence_date ON rdti_evidence(date_captured);
...
-- Create unique constraint and indexes for bas_preparations
CREATE UNIQUE INDEX IF NOT EXISTS idx_bas_preparations_unique ON bas_preparations(entity_code, bas_quarter);
CREATE INDEX IF NOT EXISTS idx_bas_preparations_entity ON bas_preparations(entity_code);
CREATE INDEX IF NOT EXISTS idx_bas_preparations_quarter ON bas_preparations(bas_quarter);
CREATE INDEX IF NOT EXISTS idx_bas_preparations_status ON bas_preparations(status);
...
-- Create indexes for cashflow_forecasts
CREATE INDEX IF NOT EXISTS idx_cashflow_forecasts_entity ON cashflow_forecasts(entity_code);
CREATE INDEX IF NOT EXISTS idx_cashflow_forecasts_date ON cashflow_forecasts(forecast_date);
CREATE INDEX IF NOT EXISTS idx_cashflow_forecasts_scenario ON cashflow_forecasts(scenario);
...
CREATE INDEX IF NOT EXISTS idx_policy_violations_item ON policy_violations(item_id);
...
-- Create unique constraint and indexes for agent_metrics
CREATE UNIQUE INDEX IF NOT EXISTS idx_agent_metrics_unique ON agent_metrics(agent_name, metric_date);
...
-- Create indexes for performance on xero_transactions
...
CREATE INDEX IF NOT EXISTS idx_xero_transactions_date ON xero_transactions(date);
CREATE INDEX IF NOT EXISTS idx_xero_transactions_amount ON xero_transactions(amount);
...
DROP TRIGGER IF EXISTS update_xero_transactions_updated_at ON xero_transactions;
CREATE TRIGGER update_xero_transactions_updated_at
