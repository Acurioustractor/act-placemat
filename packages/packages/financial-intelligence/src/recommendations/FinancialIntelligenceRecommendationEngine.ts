...
export class FinancialIntelligenceRecommendationEngine {
  ...
  private async initializeAsync() {
    try {
      // Dynamically import cache manager (Node.js specific)
      if (typeof window === 'undefined') {
        const { default: cacheManager } = await import('../../../../apps/backend/src/services/intelligentCacheManager.js');
        this.cacheManager = cacheManager;
      }
      this.initialized = true;
    } catch (error) {
      console.warn('Cache manager not available, running without cache:', error.message);
    }
  }
  ...
  public async generateRecommendations(context: AnalysisContext): Promise<FinancialRecommendation[]> {
    ...
    if (this.cacheManager) {
      try {
        const cached = await this.cacheManager.get('recommendations', contextHash, {
          metrics: 'financial',
          version: '1.0'
        });
        
        if (cached && Array.isArray(cached)) {
          console.log('📦 Using cached financial recommendations');
          return cached.filter(r => new Date(r.validUntil) > new Date());
        }
      } catch (error) {
        console.warn('Cache retrieval failed, generating fresh recommendations:', error.message);
      }
    }
    ...
    if (this.cacheManager && finalRecommendations.length > 0) {
      try {
        await this.cacheManager.set('recommendations', contextHash, finalRecommendations, {
          metrics: 'financial',
          version: '1.0'
        }, 600); // 10 minute cache
        
        console.log('💾 Cached financial recommendations');
      } catch (error) {
        console.warn('Failed to cache recommendations:', error.message);
      }
    }
  ...
  private hashContext(context: AnalysisContext): string {
    // Create a stable hash from key financial metrics
    const keyMetrics = {
      cashBalance: Math.round(context.financialMetrics.cashBalance),
      monthlyRevenue: Math.round(context.financialMetrics.monthlyRevenue),
      monthlyExpenses: Math.round(context.financialMetrics.monthlyExpenses),
      runwayDays: Math.round(context.financialMetrics.runwayDays),
      errorRate: Math.round(context.systemPerformance.errorRate * 10000), // Precision for small numbers
      complianceFlags: [
        context.complianceStatus.privacyActCompliant,
        context.complianceStatus.austracCompliant,
        context.complianceStatus.acncCompliant,
        context.complianceStatus.careCompliant
      ].join('')
    };

    // Simple hash function - in production, consider using crypto.createHash
    let hash = 0;
    const str = JSON.stringify(keyMetrics);
