...
/**
 * Populate Intelligence Data for ACT Platform v3
 * 
 * This script populates the intelligence tables with initial data
 * to make the Business Agent v3 and CRM System v3 more useful
 */
...
console.log('🚀 Populating Intelligence Data for ACT Platform v3');
console.log('====================================================');
...
async function populateContactIntelligence() {
  ...
  const { error: insertError } = await supabase
    ...
    .upsert(intelligenceData, { onConflict: 'contact_id' });
...
async function populateProjectMatches() {
  ...
  const { error: insertError } = await supabase
    ...
    .upsert(matches, { onConflict: 'project_id,contact_id' });
