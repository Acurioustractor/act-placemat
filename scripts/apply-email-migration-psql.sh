...
# Apply Email Financial Intelligence Migration via psql
# Direct database connection - bypasses Supabase Studio

set -e

MIGRATION_FILE="/Users/benknight/Code/ACT Placemat/supabase/migrations/20260130000002_email_financial_intelligence.sql"

echo "========================================"
echo "Applying Email Financial Intelligence Migration"
echo "========================================"
...
if [ -z "$SUPABASE_DB_PASSWORD" ] && [ -z "$DB_PASSWORD" ]; then
  ...
  echo "  SUPABASE_DB_PASSWORD='your_password' ./scripts/apply-email-migration-psql.sh"
  echo ""
...
if [ $? -eq 0 ]; then
  ...
  PGPASSWORD="$PASSWORD" psql \
    ...
    -U "$DB_USER" \
    -d "$DB_NAME" \
    -c "SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'email_financial_documents';"

  echo ""
  echo "✅ READY TO EXTRACT SUBSCRIPTIONS!"
  echo ""
  echo "Run next:"
  echo "  node apps/backend/extract-all-subscriptions.js"
  echo ""
